...
The original study (n = 112) found that the interaction between the measure of inherence bias and behavior typicality was a significant predictor of the measure of ought inferences ($b$ = 2.44, 95% CI = [.78, 4.10]). The original study used a linear mixed-effects model. I estimated the effect size by calculating Cohens $d_z$ for the difference between the predictor and 0 ($d_z = 0.27$). The original power to detect this effect was .81. To achieve 80% power, a sample size of 110 is needed. To achieve 90% power, a sample size of 147 is needed. To achieve 95% power, a sample size of 181 is needed. 

###Planned Sample

The planned sample size is 130. 
...
#run funciton for all json files and bind them into a single data frame
...
The following creates a tibble of the inherence scores for each participant:
```{r}
inherence_scores <-
  data %>% 
  group_by(id) %>% 
...
The following creates a tibble of the CRT for each participant:
```{r}
...
The following removes the attention checks prompts so that they are not included in the analysis. add in inherence scores and crt scores and typicality. 
```{r}
data_filtered <-
  data %>%
  filter(prompt != attention_check_inherence,
...
Join the data with the inherence scores and crt scores:
```{r}
data_scores <-
  data_filtered %>%
  group_by(id) %>% 
...
Add in binary typicality measure for each prompt:
```{r}
data_final <-
  data_scores %>% 
...
fit <- lmer(value ~ inherence_measure*typicality + crt_score*typicality + education*typicality + political*typicality + (1 | id) + (1 | prompt), data = data_final)
```
