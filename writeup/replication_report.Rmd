...
get_data_from_json <- function(data_path) {
  ...
  ought_data <-
    tibble(
      ...
      #age = as.integer(d$answers$data$age),
      #gender = d$answers$data$gender,
      #race = d$answers$data$ethnicity,
      #comments = d$answers$data$comments
      ) %>% 
    group_by(prompt) %>% 
...
inherence_scores <-
  ...
  summarise(inherence_bias = sum(intrinsic) - sum(extrinsic))
```
...
by_ib_group %>% 
  ...
  coord_cartesian(ylim = c(40, 60)) + #this will change--it's too hard to see the errorbars otherwise
